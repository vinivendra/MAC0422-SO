...
PUBLIC int do_lsr()
{
    struct fproc *fp_atual;
    register struct inode *rip;
    int i = 0;
    int j = 0;
    int open_result = -14;
    fetch_name(m_in.name1, m_in.name1_length, M1);
    
    printf("OK AGORA TEM INFORMACAO PODE PEGAR SO QUE SOH O NOME u_u\n");
    ...
    if (rip == NIL_INODE) {
        printf("DAAAAAAAAAAAAAAAARKNESS\n");
        return -1;
    }
    
    printf("OW DEU CERTO QUEM DIRIA! OK, VAMU LA:\n");
    
    for (i = 0; i < NR_PROCS; i++) {
        fp_atual = &fproc[i];
        
        for (j = 0; j < OPEN_MAX; j++) {
            if (fp_atual->fp_filp[j] != NIL_FILP) {
                if (fp_atual->fp_filp[j]->filp_ino == rip) {
                    printf("ACHEI UM: %d\n", fp_atual->fp_pid);
                }
            }
        }
...
PUBLIC int do_open()
{
    ...
    if (m_in.mode & O_CREAT || m_in.mode & O_CREATI) {
...
PRIVATE int common_open(register int oflags, mode_t omode)
{
    ...
    if (oflags & O_CREAT) {
        ...
        printf("CRIEI UM IMEDIATO UOOOOOOOOOOOOU D:\n");
        /* Create a new inode by calling new_node(). */
