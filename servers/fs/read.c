...
int rw_flag;			/* READING or WRITING */
{
    ...
    /* ##################################################### */
    if (mode_word == I_IMMEDIATE) {printf("Reading/writing to an IMMEDIATLE FILE!\n");}
    
    /* Checa se é um arquivo imediato, que deve ser tradado do mesmo modo que arquivos regulares. */
    ...
    if (char_spec) {
        ...
        /* Checa se o arquivo é imediato */
        if ((rip->i_mode & I_TYPE) == I_IMMEDIATE) {
            chunk = (int) m_in.nbytes;      /* Número de bytes */
            
            if (rw_flag == WRITING) {       /* Se vamos escrever */
                int novoTamanho = f_size + m_in.nbytes;
                
                if (novoTamanho > 32) {     /* Se vamos passar de 32 bytes, precisa transformar em arquivo normal */
                    /* Passa as coisas do inode pro buffer */
                    /* Escreve como se fosse inode normal */
                }
                else {
                    /* Copia as informações do buffer do m_in pro i_zone do inode */
                    r = sys_vircopy(usr,
                                    seg,
                                    (phys_bytes) m_in.buffer,
                                    FS_PROC_NR,
                                    D,
                                    (phys_bytes) ((char *)rip->i_zone + position),
                                    (phys_bytes) chunk);
                    /* inode tá sujo, seta o dirty bit */
                    rip->i_dirt = DIRTY;
                    ...
                    if (r == OK) {
                        /* Se deu erro */
                        if (rdwt_err < 0) {
                            printf("Erro na leitura/escrita do arquivo imediato.");
                        }
                        else {
                            m_in.buffer += chunk;
                            cum_io += chunk;
                ...
                /* Se o ponteiro do arquivo já passou do EOF, cancela */
                if (position >= f_size) {
                    m_in.nbytes = 0;
                }
                else {
                    /* Se vai passar do EOF com essa leitura, lê só até o EOF */
                    bytes_left = f_size - position;
                    if (chunk > bytes_left) {
                        chunk = (int) bytes_left;
                    }
                    
                    /* Passa do iNode para o buffer do m_in */
                    r = sys_vircopy(FS_PROC_NR,
                                    D,
                                    (phys_bytes) ((char *)rip->i_zone + position),
                                    usr,
                                    seg,
